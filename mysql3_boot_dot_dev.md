...
❗ In sql we terminate statements with semicolon *;*
...
- `GENERATED ALWAYS AS IDENTITY` prevents manual insertion into the id column.
...
When setting a foreign key while creating table we need to choose what will happen with that field when row with 
associated primary key is deleted. We can either set it to null (ON DELETE SET NULL), or we can delete the whole table row (CASCADE)
...
In other than sqllite dialects of SQL you can ADD CONSTRAINT within an ALTER TABLE statement. SQLite does not support this feature, so when we create our tables we need to make sure we specify all the constraints we want.
...
YOUR DB*. You also do not want to delete data base on the name, or any other non-unique field, as you can accidentally remove more fields than intended. Probably the best strategy is to simply use primary key.
...
```sql
    UPDATE users
    SET name = 'John Doe' WHERE name IS NULL;
```

```sql
    UPDATE Employees
    SET Salary = Salary * 1.10, JobTitle = 'Manager' WHERE EmployeeID = 5;
```
...
- returns *one row for each unique value*
